...
HERMOD_REMOTE=wss://… HERMOD_API_TOKEN=… hermod mcp
```

The bearer token lives at `$HERMOD_HOME/identity/api_token` on the
daemon host (mode 0600, generated by `hermod init`). Each user runs
their own daemon (locally or in cloud); multi-user collaboration
happens via federation between daemons, see `DEPLOY.md` §2 and §3.
