...
//!
//! Functional twin of `backends::sqlite::SqliteCapabilityRepository`. The
//! `list` method's dynamic WHERE-clause builder tracks the next
//! parameter index manually because Postgres requires `$N`
//! placeholders to be numbered (SQLite's `?` is positional and would
//! re-number automatically on a string append).
...
impl CapabilityRepository for PostgresCapabilityRepository {
    ...
    async fn revoke(&self, id: &str, at: Timestamp) -> Result<bool> {
        let res = sqlx::query(
            r#"UPDATE capabilities SET revoked_at = $1 WHERE id = $2 AND revoked_at IS NULL"#,
        )
        .bind(at.unix_ms())
        .bind(id)
        .execute(&self.pool)
    ...
    async fn is_revoked(&self, id: &str) -> Result<bool> {
        let row = sqlx::query(r#"SELECT revoked_at FROM capabilities WHERE id = $1"#)
            .bind(id)
    ...
    async fn list(
        ...
        // $1, $2 always present (self_id + direction). Subsequent
        // conditional binds increment `next_param` so the placeholder
        // numbering stays in lock-step with the bind order.
        let mut sql = format!(
            "SELECT id, issuer, audience, scope, target, expires_at, revoked_at, raw_token \
             FROM capabilities \
             WHERE {pivot_col} = $1 AND direction = $2",
        );
        let mut next_param: u32 = 3;
        ...
        if filter.after_id.is_some() {
            use std::fmt::Write;
            let _ = write!(sql, " AND id > ${next_param}");
            next_param += 1;
        }
        sql.push_str(" ORDER BY id ASC");
        if filter.limit.is_some() {
            use std::fmt::Write;
            let _ = write!(sql, " LIMIT ${next_param}");
        }
...
fn row_to_capability(row: sqlx::postgres::PgRow) -> Result<CapabilityRecord> {
