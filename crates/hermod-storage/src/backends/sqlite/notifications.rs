...
//!
//! Atomicity discipline mirrors the messages outbox: claim is one
//! `UPDATE … RETURNING` statement under SQLite's writer lock so two
//! workers can't observe the same row as `pending`. The cap-and-INSERT
//! check at enqueue time is a single statement (`INSERT … SELECT … WHERE
//! (SELECT COUNT … ) < cap`) so a sustained burst can't punch through
...
impl NotificationRepository for SqliteNotificationRepository {
    async fn enqueue(&self, req: &EnqueueRequest, max_pending: u32) -> Result<EnqueueOutcome> {
        // INSERT … SELECT … WHERE (cap-check) < cap.
        // Returning rows_affected = 0 ⇒ cap-blocked; 1 ⇒ inserted.
        let res = sqlx::query(
    ...
    async fn list(
        ...
        let placeholder = match statuses {
...
fn row_to_notification(row: sqlx::sqlite::SqliteRow) -> Result<NotificationRecord> {
