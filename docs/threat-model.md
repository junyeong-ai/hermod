...
| T6  | Local malware reading identity / messages | `$HERMOD_HOME/identity/` written with mode 0700; every secret file (`ed25519_secret`, `tls.key`, `bearer_token`) written with mode 0600. `identity::layout::enforce` re-checks every spec'd file at daemon boot and refuses to start on a mode breach (sshd `StrictModes` model — no silent repair so an attacker who chmod-relaxed a file can't have the daemon "fix" it back). SQLite DB inherits user perms. `SecretString` (in-memory bearer + audit-webhook token) and `Keypair`/`WorkspaceSecret`/`ChannelMacKey` derive `ZeroizeOnDrop`. |
