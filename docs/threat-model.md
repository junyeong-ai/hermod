...
| T13 | Multi-device confusion | Identity is two-tier: `hosts.id = base32(blake3(host_pubkey))[:26]` for daemons (the entity authenticated by federation Noise XX), `agents.id = base32(blake3(agent_pubkey))[:26]` for envelope senders/receivers. The two tables FK via `agents.host_id`. Two daemons with the same host seed are by design the same host; two agent installs with the same agent seed are the same agent. Distinct identities require distinct seeds. TLS-fingerprint TOFU pins the host-level cert (one cert per daemon, shared by every agent it carries). |
...
| T18 | Alias spoofing / impersonation | Identity is the ed25519 hash; aliases are display only. Routing (`--to @alias`) consults `local_alias` exclusively — receiver-set, sacred, UNIQUE per daemon. A peer's self-asserted alias lands in `peer_asserted_alias` (advisory, no UNIQUE) and never overrides the local label. Federation handshake / mDNS TXT collisions on a local label drop the peer's `local_alias` claim at `agents().upsert_observed()` (the receiver-sovereignty path) — only the `peer_asserted_alias` slot updates. Operator-driven paths (`peer add --alias`, `agent register`) surface a UNIQUE-constraint storage error on collision so the operator picks a different alias before the row lands. Channel notifications expose `from_local_alias` + `from_peer_alias` separately so consumers can detect a mismatch. |
...
| T28 | Operator-configured auto-approve weakens the trust floor | `[[auto_approve.confirmation]]` rules let an operator downgrade `Verdict::Confirm` ⇒ `Accept` for inbounds that match a condition (e.g. `KindIn { kinds: [Direct] }` AND `TrustIn { trusts: [Verified] }`). Two failure modes: (1) the operator writes a rule that fires too liberally (e.g. matches all inbounds regardless of trust) and silently auto-accepts dangerous content; (2) the overlay machinery accidentally crosses the matrix floor (downgrades `Reject` to `Accept`). (1) is mitigated by the audit trail: every overlay match emits `confirmation.auto_accept` with the matched rule's `name`, the inbound's `kind`/`sensitivity`/`trust_level`, and the envelope id; an operator scanning the log for "rule X fired N times" sees over-firing and tightens the rule. (2) is **architecturally impossible**: `AutoApproveOutcome` has only `NoOp`, `Accept { rule }`, and `Allow { rule }` variants; there is no variant that escalates `Reject`. The overlay is invoked exclusively inside the `Verdict::Confirm` arm of `confirmation::decide`, enforced by `scripts/check_trust_boundaries.sh` (mechanical grep on the inbound source). Boot validation (`AutoApproveConfig::validate`) rejects empty rule names, duplicate rule names, and any condition that the dispatch validator would reject (oversized keywords, etc.) — typos abort startup. |
| T29 | Permission auto-allow weakens tool-call gating / RCE risk | `[[auto_approve.permission]]` rules let an operator pre-approve specific `(origin, tool_name)` pairs; the daemon resolves the request with `Allow` immediately on match. The risk is twofold: (1) auto-allowing a destructive tool (`Bash`, `Write`, `Edit`, `NotebookEdit`) hands a peer host-level RCE; (2) an LLM-craftable matcher (regex/glob/`input` predicate) lets a peer engineer requests that match anything. Both are foreclosed by the rule shape: `tool_names` is a *Vec\<String>* allowlist (no regex / glob / pattern), `origin` is a stable `AgentId` (not the mutable alias), and there is no `input` field on `PermissionRule`. Compile-time `FORBIDDEN_TOOL_NAMES = ["Bash", "Write", "Edit", "NotebookEdit"]`: any rule that lists one of these aborts daemon boot via `AutoApproveConfig::validate`. The forbidden list has **no escape hatch** — no `--i-accept-the-risk` flag, no env override, no per-rule override. An operator who genuinely wants hands-off shell execution runs a shell, not Claude Code; auto-approving destructive tools on Hermod's surface defeats Claude Code's prompt purpose. The rule shape, forbidden list, and downgrade-only outcome enum are all pinned by `scripts/check_trust_boundaries.sh` (mechanical grep) so a future commit that introduces a regex field or removes a forbidden tool fails CI. Per-session isolation (T25) still applies — auto-allow respects `owner_session`, so sibling Claude Code windows of the same agent never observe each other's auto-allowed verdicts. |
| T30 | Capability tags coerced into a trust signal | Tags propagate via `peer.advertise` and surface in `agent.list --tag-…` / `agent.get` for discovery — "find me a rust+tokio peer". Risk: a future commit (or a confused contributor) starts treating `peer_asserted_tags` as authority — auto-trusting a peer whose tags include `verified:true`, or letting an inbound rule fire on `peer.tags.contains("admin")`. The structural mitigation: `hermod-routing` imports zero `capability_tag` / `CapabilityTag` symbols. `scripts/check_trust_boundaries.sh` enforces this via grep over the entire `crates/hermod-routing/src/` tree (test fixtures stripped via `#[cfg(test)]` truncation). A commit that adds even one `use hermod_core::CapabilityTag` to the routing crate's production code fails CI before review. Tags are storage-and-display only — `agent.list --tag-…` filters happen in `services::agent`, which is a daemon-side service, not a routing decision. The peer-asserted side is also bounded: `CapabilityTagSet::parse_lossy` drops malformed entries per-entry rather than rejecting the whole row, so a peer shipping garbage can't poison the directory or crash the receiver, and the dropped count surfaces on the `peer.advertise.received` audit row (`dropped_invalid_tags`). Cardinality is capped at 16 per agent, length at 64 bytes per tag — bounded wire bloat, bounded memory. |
